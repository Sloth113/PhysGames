...
void PhysicsDemo::update(float deltaTime) {
	...
	glm::vec2 mousePos = glm::vec2((float)input->getMouseX() / getWindowWidth() * 1000, (float)input->getMouseY() / getWindowHeight() * 1000 / aspRatio);
	aie::Gizmos::add2DCircle(mousePos, 1, 16, glm::vec4(0, 1, 1, 1));

	//Spawn ball on left click 
	if (input->isMouseButtonDown(aie::INPUT_MOUSE_BUTTON_LEFT)) {
		m_ballSize += deltaTime * 50;
		aie::Gizmos::add2DCircle(mousePos, m_ballSize, 32, glm::vec4(1, 0, 1, 1));
	}
	if (input->wasMouseButtonReleased(aie::INPUT_MOUSE_BUTTON_LEFT) && m_ballSize > 0) {
		Sphere * ball = new Sphere(mousePos, glm::vec2(0, 0), m_ballSize *m_ballSize / 100, m_ballSize, glm::vec4(0, 0, 1, 1));
		...
		if (m_hitThis == nullptr) {
			for (PhysicsObject * p : (m_physicsScene->getActors())) {
				if (Rigidbody * r = dynamic_cast<Rigidbody*>(p)) {
					Sphere * tmp = new Sphere(mousePos, glm::vec2(0, 0), 1, 1, glm::vec4(0, 1, 1, 1));
			...
			aie::Gizmos::add2DLine(m_clickPos, mousePos, glm::vec4(0, 1, 1, 1));
		}
	}
	if (input->isMouseButtonUp(aie::INPUT_MOUSE_BUTTON_RIGHT) && m_clickPos != glm::vec2(0, 0)) {
		...
		m_hitThis->applyForce((mousePos - m_clickPos) * 10.0f, m_clickPos - m_hitThis->getPosition());
		m_clickPos = glm::vec2(0, 0);
		m_hitThis = nullptr;
	}
	...
	if (input->isMouseButtonDown(aie::INPUT_MOUSE_BUTTON_MIDDLE)) {
		if (m_moveThis == nullptr) {
			...
			m_moveThis->setPosition(mousePos);
		}
	}
...
void PhysicsDemo::Platformer()
{
	...
	*/
	...
	DeathBox * dBox = new DeathBox(glm::vec2(500, 200), glm::vec2(0, 0), 0, 10, glm::vec2(100, 100), glm::vec4(1, 0, 0, 0.2f));
	dBox->setKinematic(true);
	m_physicsScene->addActor(dBox);
}

void PhysicsDemo::BallandCorner()
{
	m_physicsScene = new PhysicsScene();
	m_physicsScene->setTimeStep(0.01f);
...
//old
void PhysicsDemo::SphereWallsCollisions()
{
	m_physicsScene = new PhysicsScene();
